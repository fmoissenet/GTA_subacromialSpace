function [Center,Radius,Error] = sphereFit(X)
...
% Update: Add the computation of the related error for each point of the
% cloud (F. Moissenet, University of Geneva)
...
Error.point=sqrt(sum([X(:,1)-Center(1),X(:,2)-Center(2),X(:,3)-Center(3)].^2,2))-Radius;
Error.mean=mean(Error.point);
Error.std=std(Error.point);